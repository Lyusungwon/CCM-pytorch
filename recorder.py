...
class Recorder:
    ...
    def epoch_start(self, epoch_idx, is_train, loader):
        ...
        self.dataset_size = len(loader.sampler) * self.batch_access
        self.epoch_loss = 0
        self.epoch_pp = 0
    ...
    def batch_end(self, batch_idx, batch_size, loss, pp):
        ...
        if self.mode == 'Train' and batch_idx % self.log_interval == 0:
            print('Train Batch: {} [{}/{}({:.0f}%)] Loss:{:.4f} / Time:{:.4f}'.format(
                ...
                self.batch_loss,
                self.batch_time))
            batch_record_idx = (self.epoch_idx - 1) * (self.dataset_size//(self.log_interval*self.batch_size)) + batch_idx // self.log_interval
